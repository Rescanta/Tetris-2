...
//   OO  shape      6,0; 6,1
//   00             7,0; 7,1
public class OBlock implements InterfaceBlock {
    //initial coordinates
    //int[][] arr = new int[4][2];
    ...
    public OBlock(int cubeX, int cubeY, int cubeSize, int cubeXMax, int cubeYMax) {
    ...
    public boolean moveNONE(String direction, int cubeSize, int cubeXMax, int cubeYMax, Terrain terrain) {
        int temp;
        if (arr.get(2).get(1) + 1 == cubeYMax || arr.get(3).get(1) + 1 == cubeYMax) {
            stuck = true;
            return stuck;
        }
    ...
    public boolean moveRIGHT(String direction, int cubeSize, int cubeXMax, int cubeYMax, Terrain terrain) {
        int temp;
        if (arr.get(1).get(0) + 1 == cubeXMax || arr.get(3).get(0) + 1 == cubeXMax) {
            //stuck = true;
            return stuck;
        }
        if (terrain.blockAndTerrainTouches(arr, direction)) {
...
//        }
    ...
    public boolean moveLEFT(String direction, int cubeSize, int cubeXMax, int cubeYMax, Terrain terrain) {
        int temp;
        if (arr.get(0).get(0) - 1 < 0 || arr.get(2).get(0) - 1 < 0) {
            //stuck = true;
            return stuck;
        }
        if (terrain.blockAndTerrainTouches(arr, direction)) {
